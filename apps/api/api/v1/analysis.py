...
async def run_analysis(deck_id: str, payload: AnalysisStart, current_user: User = Depends(get_current_user)):
    # Trigger Celery
...
async def debug_parse_deck(file: UploadFile = File(...), current_user: User = Depends(get_current_user)):
