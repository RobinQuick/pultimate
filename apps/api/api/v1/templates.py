...
async def create_template(
    ...
    try:
        ...
        new_template = Template(workspace_id=workspace_id, name=name)
        db.add(new_template)
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"Template processing failed: {str(e)}") from e
        ...
        raise HTTPException(status_code=500, detail=f"Template creation failed: {str(e)}") from e
    finally:
        # Clean up temp file
...
async def list_templates(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(Template).where(Template.workspace_id == "default-ws"))
    templates = result.scalars().all()

    return [{"id": t.id, "name": t.name, "is_archived": t.is_archived} for t in templates]


@router.get("/{template_id}")
async def get_template(
    template_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    ...
    result = await db.execute(
        select(Template).where(Template.id == template_id).options(selectinload(Template.versions))
    )
