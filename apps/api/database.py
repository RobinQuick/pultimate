...
def get_async_engine():
    ...
    return create_async_engine(db_url, echo=False, connect_args={"ssl": ssl_context})
...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            result = await session.execute(select(models.Tenant).where(models.Tenant.id == "default-tenant"))
            tenant = result.scalars().first()

            if not tenant:
                tenant = models.Tenant(id="default-tenant", name="Default Tenant", slug="default")
                session.add(tenant)
                await session.flush()
                print("✓ Created default tenant")

            # Check if default workspace exists
            result = await session.execute(select(models.Workspace).where(models.Workspace.id == "default-ws"))
            workspace = result.scalars().first()

            if not workspace:
                workspace = models.Workspace(id="default-ws", tenant_id="default-tenant", name="Default Workspace")
